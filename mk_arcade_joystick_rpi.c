...
#define START_READ BSC_C_I2CEN|BSC_C_ST|BSC_C_CLEAR
#define START_WRITE BSC_C_I2CEN|BSC_C_ST|BSC_C_READ
...
static int __init mk_setup_pad(struct mk *mk, int idx, int pad_type_arg) {
	...
	if (pad_type != MK_ARCADE_MCP23017 && pad_type != MK_TEENSY) {
		...
		i2c_write(pad->i2caddr, MPC23017_GPIOA_PULLUPS_MODE, &FF, 1, &timeout);
		udelay(1000);
		// Put all GPIOB inputs on MCP23017 in INPUT mode
		i2c_write(pad->i2caddr, MPC23017_GPIOB_MODE, &FF, 1, &timeout);
		udelay(1000);
		// Put all inputs on MCP23017 in pullup mode
